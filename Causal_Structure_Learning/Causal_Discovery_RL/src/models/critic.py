...
class Critic(object):
    ...
    def predict_rewards(self, encoder_output, cor_mat):
        # [Batch size, Sequence Length, Num_neurons] to [Batch size, Num_neurons]
        frame = tf.reduce_mean(encoder_output, 1)

        # [Batch size, m]
        h1 = tf.reshape(cor_mat, [self.batch_size, -1])
        h1 = tf.layers.dense(h1, self.input_embed//2, activation=tf.nn.relu, kernel_initializer=self.initializer)
        frame = tf.concat([frame, h1], 1)
