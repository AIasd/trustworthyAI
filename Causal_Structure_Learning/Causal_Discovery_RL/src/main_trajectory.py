...
# def build_target_update(from_scope, to_scope, TAU=8e-2):
    ...
    with tf.Session(config=sess_config) as sess:
        ...
        for i in (range(1, config.nb_epoch + 1)):
            ...
            for j in range(1, config.trajectory_step+1):
                ...
                states_np.append(input_batch)
                states_cor_mat.append(graph_last_timestep)

                graphs_feed, graph_last_timestep = sess.run([actor.graphs, actor.scores], feed_dict={actor.input_: input_batch, actor.graph_last_timestep:graph_last_timestep})
                ...
                # Get feed dict
                feed = {actor.input_: input_batch, actor.reward_: -reward_feed[:,0], actor.graphs_:graphs_feed, actor.graph_last_timestep: graph_last_timestep, actor.V_end_:np.zeros(config.batch_size)}


                advantage = sess.run([actor.reward_baseline], feed_dict=feed)


                actions.append(graphs_feed)
                rewards.append(reward_feed[:, 0])
                advantages.append(advantage)

            # Get feed dict
            # only last one step
            # print(np.array(states_np).shape, np.array(graphs_feed).shape, np.array(graph_last_timestep).shape, '\n'*50)
            feed = {actor.input_: np.array(states_np)[-1], actor.reward_: -reward_feed[:,0], actor.graphs_:graphs_feed, actor.graph_last_timestep: graph_last_timestep}

            V_end, avg_baseline = sess.run([actor.critic.predictions, actor.avg_baseline], feed_dict=feed)
            gamma = 0
            # print('\n'*20, np.array(rewards).shape, np.array(avg_baseline).shape, np.array(V_end).shape)
            V_value = discounted_rewards(rewards-avg_baseline, gamma, V_end)
            ...
            for subiter in range(config.trajectory_step):
                ...
                feed1 = {actor.input_: np.array(states_np[ind_array]),  actor.graphs_:graphs_feed, actor.graph_last_timestep: np.array(states_cor_mat[ind_array]), actor.V_end_: V_value[ind_array]}
                train_step3 = sess.run([actor.train_step3], feed_dict=feed1)


            # get new v value
            Computed_Vnew=[]
            for subiter in range(config.trajectory_step):
                feed = {actor.input_: np.array(states_np[subiter*config.batch_size:(subiter+1)*config.batch_size]), actor.reward_: -reward_feed[:,0], actor.graphs_:graphs_feed, actor.graph_last_timestep: states_cor_mat[subiter*config.batch_size:(subiter+1)*config.batch_size]}
            ...
            rewards = np.reshape(np.array(rewards), [config.batch_size*config.trajectory_step])

            # Get feed dict
            for subiter in range(config.trajectory_step):
                ind_array = np.random.choice(range(config.batch_size * config.trajectory_step), config.batch_size)

                feed2 = {actor.input_: np.array(states_np[ind_array]), actor.graphs_:graphs_feed, actor.graph_last_timestep: np.array(states_cor_mat[ind_array]), actor.V_end_: Computed_Vnew[ind_array], actor.reward_:rewards[ind_array]}

                summary, base_op, score_test, probs, graph_batch, \
                reward_batch, reward_avg_baseline, train_step4 = sess.run([actor.merged, actor.base_op,
                actor.test_scores, actor.log_softmax, actor.graph_batch, actor.reward_batch, actor.avg_baseline, actor.train_step4], feed_dict=feed2)
